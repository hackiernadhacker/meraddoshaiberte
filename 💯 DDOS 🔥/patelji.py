...
@bot.message_handler(commands=['Patel_ji_please_add_me', 'Nikal_laude'])
def approve_or_disapprove_user(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    is_admin = is_user_admin(user_id, ADMIN_ID)
...
🤷‍♀️𝙀𝙭𝙖𝙢𝙥𝙡𝙚👉 🆔 123456  day 499 *''', parse_mode='Markdown')
    ...
    if len(cmd_parts) < 2:
        bot.send_message(chat_id, "*patelji Invalid command format. Use /Patel_ji_please_add_me <user_id> <plan> <days> or /Nikal_laude <user_id>.*", parse_mode='Markdown')
        return
    ...
    if action == '/Patel_ji_please_add_me':
        if plan == 1: 
            if users_collection.count_documents({"plan": 1}) >= 99:
                bot.send_message(chat_id, "*ApprovalPatel_ji_please_add_me failed: patelji limit reached (99 users).*", parse_mode='Markdown')
                return
        elif plan == 2:  # patelji
            if users_collection.count_documents({"plan": 2}) >= 499:
                bot.send_message(chat_id, "*Patel_ji_please_add_me failed: patelji limit reached (99 users).*", parse_mode='Markdown')
                return
